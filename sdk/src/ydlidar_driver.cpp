...
int YDlidarDriver::cacheScanData() {
  ...
  while (isScanning) {
      ...
      if (IS_FAIL(ans) || timeout_count > DEFAULT_TIMEOUT_COUNT) {
        if (!isAutoReconnect) {
          ...
          if (IS_OK(ans)) {
            ...
            local_scan[0].single_package_flag = Node_NotSync;
          } else {
            isScanning = false;
            return RESULT_FAIL;
          }
    ...
    for (size_t pos = 0; pos < count; ++pos) {
      if (local_buf[pos].single_package_flag & LIDAR_RESP_MEASUREMENT_SYNCBIT) {
        if ((local_scan[0].single_package_flag & LIDAR_RESP_MEASUREMENT_SYNCBIT)) {
...
result_t YDlidarDriver::waitPackage(node_info *node, uint32_t timeout) {
  ...
  (*node).single_package_flag = Node_NotSync;
  ...
  if (package_Sample_Index >= nowPackageNum) {
    ...
    if ((*node).sync_flag != Node_Sync && package_Sample_Index > 1) {
      (*node).single_package_flag = Node_Sync;
    }
